...
class AllExchangesReconnectTester:
    ...
    def __init__(self, use_testnet: bool = False):
        ...
        # 거래소별 웹소켓 인스턴스
        self.websockets: Dict[str, BaseWebsocketConnector] = {}
        
        # 거래소별 통계
        self.exchange_stats: Dict[str, ExchangeStats] = {
            "binancespot": ExchangeStats(name="binancespot"),
            "binancefuture": ExchangeStats(name="binancefuture"),
            "bybitspot": ExchangeStats(name="bybitspot"),
            "bybitfuture": ExchangeStats(name="bybitfuture"),
            "upbit": ExchangeStats(name="upbit"),
            "bithumb": ExchangeStats(name="bithumb")
        }
    ...
    def connection_status_callback(self, exchange: str, status: str):
        ...
        if status == "connect_attempt":
            self.logger.info(f"[{exchange}] 연결 시도 중...")
            ...
            if stats.connections > 1 or stats.forced_disconnects > 0:
                ...
                self.logger.info(f"[{exchange}] 재연결 성공 ({stats.reconnections}번째)")
                ...
                if exchange == "upbit":
                    self.logger.info(f"[{exchange}] 재연결 후 모든 심볼의 스냅샷 자동 수신 대기")
            else:
                self.logger.info(f"[{exchange}] 초기 연결 성공")
            ...
            self.logger.info(f"[{exchange}] 연결 끊김 ({stats.disconnections}번째)")
            ...
            self.logger.error(f"[{exchange}] 오류 발생")
            
        elif status == "snapshot_request":
            self.logger.info(f"[{exchange}] 스냅샷 요청 중...")
            ...
            if stats.reconnection_pending:
                ...
                self.logger.info(f"[{exchange}] 재연결 후 스냅샷 수신 ({stats.reconnection_snapshot_count}번째)")
                ...
                if isinstance(message, dict):
                    ...
                    if stats:
                        ...
                        if msg_type == "snapshot":
                            ...
                            # 재연결 후 스냅샷 수신인 경우
                            if stats.reconnection_pending:
                                self.logger.info(f"[{exchange}] 재연결 후 {symbol} 스냅샷 수신")
                                ...
                                if len(stats.symbols_with_snapshot) == len(EXCHANGE_SYMBOLS[exchange]):
                                    ...
                                    self.logger.info(f"[{exchange}] 모든 심볼({len(EXCHANGE_SYMBOLS[exchange])}개)에 대한 스냅샷 수신 완료")
    ...
    async def force_disconnect(self):
        ...
        # 각 거래소별 연결 끊김 횟수 추적
        disconnect_counts = {
            exchange: 0 for exchange in self.websockets.keys()
        }

        while not self.stop_event.is_set():
            try:
                # 모든 거래소가 연결될 때까지 대기
                all_connected = all(stats.is_connected for stats in self.exchange_stats.values())
                if not all_connected:
                    await asyncio.sleep(1)
                    continue
                
                # 모든 거래소가 최대 테스트 횟수에 도달했는지 확인
                if all(count >= MAX_RECONNECT_TESTS for count in disconnect_counts.values()):
                    self.logger.info(f"모든 거래소가 최대 테스트 횟수({MAX_RECONNECT_TESTS}회)에 도달하여 테스트를 종료합니다.")
                    self.stop_event.set()
                    break

                # 아직 최대 테스트 횟수에 도달하지 않은 거래소들 중에서 선택
                available_exchanges = [
                    exchange for exchange, count in disconnect_counts.items()
                    if count < MAX_RECONNECT_TESTS
                ]
                ...
                # 랜덤으로 거래소 선택 (아직 최대 횟수에 도달하지 않은 거래소 중에서)
                exchange_name = random.choice(available_exchanges)
                ws = self.websockets.get(exchange_name)
                stats = self.exchange_stats.get(exchange_name)

                if not ws or not stats or not stats.is_connected:
                    self.logger.info(f"[{exchange_name}] 이미 연결이 끊긴 상태이므로 강제 연결 끊기를 스킵합니다.")
                    continue

                # 랜덤 시간 대기
                wait_time = random.uniform(RECONNECT_INTERVAL_MIN, RECONNECT_INTERVAL_MAX)
                self.logger.info(f"[{exchange_name}] {wait_time:.1f}초 후 강제 연결 끊기 시도 예정 ({disconnect_counts[exchange_name] + 1}/{MAX_RECONNECT_TESTS}회)")
                await asyncio.sleep(wait_time)

                # 강제 연결 끊기 방법 선택
                disconnect_method = random.choice(FORCE_DISCONNECT_METHODS)
                stats.forced_disconnects += 1
                disconnect_counts[exchange_name] += 1

                self.logger.info(f"[{exchange_name}] 강제 연결 끊기 시도 ({disconnect_counts[exchange_name]}/{MAX_RECONNECT_TESTS}회, 방법: {disconnect_method})")

                if disconnect_method == "close":
                    # 정상적인 방법으로 연결 끊기
                    if ws and hasattr(ws, 'ws') and ws.ws:
                        try:
                            await ws.ws.close()
                            self.logger.info(f"[{exchange_name}] 웹소켓 close() 메소드로 연결 끊기 성공")
                        except Exception as close_e:
                            self.logger.error(f"[{exchange_name}] 웹소켓 close() 메소드 호출 중 오류: {close_e}")
                    ...
                    if ws and hasattr(ws, 'ws'):
                        try:
                            ...
                            ws.ws = None
                            self.logger.info(f"[{exchange_name}] 웹소켓 객체를 None으로 설정하여 오류 발생시킴")
                        except Exception as err_e:
                            self.logger.error(f"[{exchange_name}] 웹소켓 객체 조작 중 오류: {err_e}")

                # 재연결 대기 시간 추가 (거래소별로 다른 대기 시간 적용)
                wait_time = random.uniform(5, 15)  # 5-15초 대기
                await asyncio.sleep(wait_time)
    ...
    async def print_stats(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # 헤더 출력
                self.logger.info(f"\n{'=' * 80}")
                self.logger.info(f"테스트 통계 (경과 시간: {elapsed:.1f}초)")
                ...
                for exchange, stats in self.exchange_stats.items():
                    status = "연결됨" if stats.is_connected else "연결 끊김"
                    self.logger.info(f"[{exchange}] 상태: {status}, "
                                    f"연결: {stats.connections}, "
                                    f"재연결: {stats.reconnections}, "
                                    f"메시지: {stats.messages_received}, "
                                    f"스냅샷: {stats.snapshot_count}, "
                                    f"재연결 후 스냅샷: {stats.reconnection_snapshot_count}")
    ...
    async def create_websockets(self) -> Dict[str, Tuple[str, List[str]]]:
        ...
        # 바이낸스 선물
        self.websockets["binancefuture"] = BinanceFutureWebsocket(self.settings, testnet=self.use_testnet)
    ...
    async def run_test(self):
        """테스트 실행"""
        self.logger.info(f"모든 거래소 웹소켓 재연결 테스트 시작 (testnet: {self.use_testnet})")
...
async def main():
    ...
    # 테스터 인스턴스 생성 및 실행
    tester = AllExchangesReconnectTester(use_testnet=use_testnet)
