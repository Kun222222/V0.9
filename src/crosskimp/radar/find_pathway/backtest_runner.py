...
class KimpBacktester:
    def __init__(self, log_file_path='/Users/kun/Desktop/CrossKimpArbitrage/v0.6/src/logs/market_order_data/250401_180820_market_orderbook.log'):
        """KIMP 백테스터 초기화"""
        ...
        # 테스트 폴더 경로 설정 (절대 경로)
        test_dir = '/Users/kun/Desktop/CrossKimpArbitrage/v0.6/tests/crosskimp'
        ...
        # 로그 파일 경로
        log_file = os.path.join(logs_dir, f"backtest_performance_{current_time}.log")
        ...
        # 프로세서 초기화
        self.processor = OptimizedKimpProcessor(apply_fee_discount=True)  # 수수료 할인 없이 실제 수수료 적용
        logging.info(f"실제 수수료 적용: {self.processor.apply_fee_discount}")
        ...
        # 결과 저장 파일 설정
        self.results_file = os.path.join(results_dir, f"kimp_backtest_results_{current_time}.csv")
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if result is not None:
                            # 10% 이상 수익률 거래는 비정상으로 간주하고 필터링
                            if result.get('net_profit', 0) >= 0.1:  # 10% 이상
                                logging.warning(f"비정상 고수익 거래 무시: {result['transfer_coin']}->{result['profit_coin']}, 순이익: {result['net_profit']:.4%}")
                                continue
                            ...
                            # 기회 발견 로깅
                            price_info = ""
                            if 'dom_transfer_buy' in result and result['dom_transfer_buy'] > 0:
                                price_info = (
                                    f"전송매수: {result['dom_transfer_buy']:,.2f}원, "
                                    f"전송매도: {result['for_transfer_sell']:.6f}$, "
                                    f"수익매수: {result['for_profit_buy']:.6f}$, "
                                    f"수익매도: {result['dom_profit_sell']:,.2f}원"
                                )
                            ...
                            logging.info(
                                ...
                                f"순이익: {result['net_profit']:.4%}, {price_info}"
                            )
            ...
            with open(self.results_file, 'a', newline='') as f:
                ...
                if not file_exists:
                    writer.writerow([
                        ...
                        'dom_transfer_buy', 'for_transfer_sell',
                        'for_profit_buy', 'dom_profit_sell',
                        'total_fee'
                    ])
                ...
                writer.writerow([
                    ...
                    opportunity.get('dom_transfer_buy', 0),
                    opportunity.get('for_transfer_sell', 0),
                    opportunity.get('for_profit_buy', 0),
                    opportunity.get('dom_profit_sell', 0),
                    opportunity.get('original_fee', 0)
                ])
    ...
    def _print_sample_results(self):
        ...
        for i in range(sample_size):
            ...
            # 가격 정보 추가
            price_info = ""
            if 'dom_transfer_buy' in opp and opp['dom_transfer_buy'] > 0:
                price_info = (
                    f"전송매수: {opp['dom_transfer_buy']:,.2f}원, "
                    f"전송매도: {opp['for_transfer_sell']:.6f}$, "
                    f"수익매수: {opp['for_profit_buy']:.6f}$, "
                    f"수익매도: {opp['dom_profit_sell']:,.2f}원"
                )
            
            logging.info(
                f"{i+1}. 전송: {opp['transfer_coin']} ({opp['transfer_pair']}, {opp['transfer_premium']:.4%}), " +
                f"수익: {opp['profit_coin']} ({opp['profit_pair']}, {opp['profit_premium']:.4%}), " +
                f"순이익: {opp['net_profit']:.4%}, {price_info}"
            )
