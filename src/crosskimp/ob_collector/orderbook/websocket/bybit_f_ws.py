...
class BybitFutureWebsocket(BaseWebsocketConnector):
    ...
    async def _do_connect(self):
        ...
        logger.info(
            f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 웹소켓 연결 시도 | 시도={self.reconnect_strategy.attempt + 1}회차, timeout={timeout}초"
        )
        try:
            ...
            logger.warning(
                f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 연결 타임아웃 발생 (무시됨) | 시도={self.reconnect_strategy.attempt + 1}회차, timeout={timeout}초"
            )
    ...
    async def _handle_connection_failure(self, reason: str, exception: Optional[Exception] = None) -> bool:
        ...
        logger.warning(
            f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 연결 실패 처리 | 이유={reason}, 재연결 시도={result}, 다음 시도까지 대기={self.reconnect_strategy.next_delay():.1f}초"
        )
        return result
                ...
                if "orderbook" in topic:
                    ...
                    if len(parts) >= 3:
                        ...
                        if msg_type == "snapshot":
                            logger.info(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 스냅샷 메시지 수신: {symbol}")
    ...
    async def handle_parsed_message(self, parsed: dict) -> None:
        ...
        try:
            ...
            if not result.is_valid:
                logger.warning(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} {symbol} 오더북 업데이트 실패: {result.error_messages}")
                if symbol not in self.snapshot_pending:
                    logger.info(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} {symbol} 스냅샷 요청 시작")
                    ...
                    if snapshot:
                        ...
                        logger.info(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} {symbol} 스냅샷 초기화 완료")
                    else:
                        logger.error(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} {symbol} 스냅샷 요청 실패")
                        self.snapshot_pending.discard(symbol)
            ...
            async with self.session.get(url) as resp:
                if resp.status == 200:
                    ...
                    if data.get("retCode") == 0:
                        ...
                        self.log_error(f"{EXCHANGE_NAMES_KR[self.exchangename]} {market} 스냅샷 응답 에러: {data}")
                else:
                    self.log_error(f"{EXCHANGE_NAMES_KR[self.exchangename]} {market} 스냅샷 요청 실패: status={resp.status}")
        except Exception as e:
            self.log_error(f"{EXCHANGE_NAMES_KR[self.exchangename]} {market} 스냅샷 요청 예외: {e}")
        return None
    ...
    def _parse_snapshot_data(self, data: dict, symbol: str) -> dict:
        ...
        try:
            ...
            self.log_error(f"{EXCHANGE_NAMES_KR[self.exchangename]} {symbol} 스냅샷 파싱 실패: {e}")
            return {}
    ...
    async def _ping_loop(self) -> None:
        ...
        while not self.stop_event.is_set() and self.is_connected:
            try:
                ...
                self.log_error(f"{EXCHANGE_NAMES_KR[self.exchangename]} PING 루프 오류: {str(e)}")
    ...
    async def reconnect(self) -> None:
        ...
        try:
            ...
            if self.is_connected:
                ...
                logger.info(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 재연결 성공")
                if self.connection_status_callback:
                    self.connection_status_callback(self.exchangename, "reconnected")
            else:
                logger.error(f"{LOG_SYSTEM} {EXCHANGE_NAMES_KR[self.exchangename]} 재연결 실패")
        except Exception as e:
