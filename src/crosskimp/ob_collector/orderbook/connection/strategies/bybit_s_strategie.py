...
바이빗 현물 거래소에 대한 웹소켓 연결 관리 전략을 제공합니다.
"""
...
class BybitSpotConnectionStrategy:
    """
    바이빗 현물 웹소켓 연결 전략 클래스
    
    바이빗 현물 거래소의 웹소켓 연결, 구독, 메시지 처리 등을 담당합니다.
    """
        ...
        try:
            ...
            if "topic" in data and "data" in data and "topic" in data and "orderbook" in data["topic"]:
                ...
                if len(topic_parts) >= 3:
                    ...
                    result = {
                        "type": original_type,  # 원본 type을 그대로 유지
                        "exchange": Exchange.BYBIT_SPOT.value,
                        "symbol": symbol,
            ...
            self.logger.error(f"바이빗 현물 메시지 JSON 파싱 오류: {str(e)}")
            return {
                "type": "error",
            ...
            self.logger.error(f"바이빗 현물 메시지 처리 중 오류: {str(e)}")
            return {
                "type": "error",
