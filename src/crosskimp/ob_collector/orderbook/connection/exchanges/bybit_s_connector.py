...
class BybitSpotConnector(ExchangeConnectorInterface):
    ...
    def is_connected(self) -> bool:
        """
        현재 연결 상태 반환 - 더 엄격한 체크 추가
        
        Returns:
            bool: 연결 상태
        """
        # 웹소켓 객체가 없거나 닫혀있으면 연결되지 않은 것으로 간주
        if self.ws is None or self.ws.closed:
            if self._connected:  # 상태 불일치 감지
                self._connected = False
                if self.connection_manager:
                    self.connection_manager.update_exchange_status(self.exchange_code, False)
            return False
                ...
                while not self._stopping and self.is_connected and self.ws:
                    try:
                        ...
                        if isinstance(parsed_message, dict) and (
                            ...
                            self.logger.debug(f"{self.exchange_name_kr} 퐁 응답 수신")
                            continue
