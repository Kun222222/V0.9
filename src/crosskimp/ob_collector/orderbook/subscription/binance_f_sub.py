...
REST_URL = "https://fapi.binance.com/fapi/v1/depth"  # 스냅샷 REST API URL
...
class BinanceFutureSubscription(BaseSubscription):
    """
    바이낸스 선물 구독 클래스
    
    바이낸스 USDT-M 선물 거래소의 웹소켓 구독을 담당하는 클래스입니다.
    ...
    async def create_subscribe_message(self, symbols: Union[str, List[str]]) -> Dict:
        ...
        for symbol in symbols:
            # 바이낸스 선물은 소문자 심볼 사용, 대문자 또는 소문자로 들어올 수 있음
            # 심볼에 이미 USDT가 포함되어 있는지 확인하고 그렇지 않은 경우에만 추가
    ...
    async def process_snapshot(self, symbol: str, data: Dict) -> None:
        ...
        try:
            # 타임스탬프 및 시퀀스 추출
            timestamp = data.get("T", int(time.time() * 1000))  # 타임스탬프
    ...
    def _extract_symbol_from_message(self, data: Dict) -> Optional[str]:
        ...
        """
        try:
            # 바이낸스 선물은 's' 필드에 심볼 정보 포함 (예: "BTCUSDT")
