...
바이빗 선물 거래소의 오더북 데이터 처리 로직을 제공합니다.
"""
...
class BybitFutureDataHandler:
    """
    바이빗 선물 데이터 핸들러 클래스
    
    바이빗 선물 거래소의 데이터 처리를 담당합니다.
    """
    ...
    async def process_snapshot(self, symbol: str, data: Dict) -> None:
        ...
        try:
            ...
            self.logger.error(f"바이빗 선물 스냅샷 처리 중 오류: {str(e)}")
        ...
        try:
            ...
            if orderbook["bids"] and orderbook["asks"]:
                ...
                if highest_bid >= lowest_ask:
                    # self.logger.warning(f"바이빗 선물 {symbol} 가격 역전 현상 감지 및 수정 중: 최고 매수가({highest_bid}) >= 최저 매도가({lowest_ask})")
                    
                    # 방법 1: 역전된 매수가 제거
                    invalid_bids = [price for price in orderbook["bids"].keys() if price >= lowest_ask]
                    for price in invalid_bids:
                        # self.logger.debug(f"바이빗 선물 {symbol} 역전된 매수가 제거: {price}")
                        del orderbook["bids"][price]
                    ...
                    if orderbook["bids"]:  # 매수가 전부 제거된 경우를 대비
                        ...
                        for price in invalid_asks:
                            # self.logger.debug(f"바이빗 선물 {symbol} 역전된 매도가 제거: {price}")
                            del orderbook["asks"][price]
