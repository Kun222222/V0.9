...
class BithumbSpotOrderBook(OrderBook):
    ...
    def __init__(self, symbol: str, depth: int = 100, logger=None):
        """초기화"""
    ...
    async def update(self, data: dict) -> ValidationResult:
        ...
        try:
            ...
            # 이전 상태 백업 (롤백용)
            prev_bids = self.bids_dict.copy()
            prev_asks = self.asks_dict.copy()
            ...
            # 역전 감지 (최고 매수가 > 최저 매도가)
            if self.cross_detection_enabled and self.bids_dict and self.asks_dict:
                highest_bid = max(self.bids_dict.keys())
                lowest_ask = min(self.asks_dict.keys())
                
                if highest_bid >= lowest_ask:
                    self.logger.warning(
                        f"[BithumbSpot] {self.symbol} 가격 역전 감지: "
                        f"최고 매수가({highest_bid}) >= 최저 매도가({lowest_ask})"
                    )
                    # 역전 상태 롤백
                    self.bids_dict = prev_bids
                    self.asks_dict = prev_asks
            ...
            if self.bids_dict and self.asks_dict:
                ...
                if spread_pct > 5:  # 스프레드가 5% 이상인 경우
                    self.logger.warning(
                        f"[BithumbSpot] {self.symbol} 비정상 스프레드 감지: "
                        f"{spread_pct:.2f}% (매수: {highest_bid}, 매도: {lowest_ask})"
                    )
            
            # 오더북 크기 로깅
            self.logger.debug(
                f"[BithumbSpot] {self.symbol} 오더북 업데이트 완료: "
                f"매수 {len(self.bids_dict)}개, 매도 {len(self.asks_dict)}개"
            )
            
            return ValidationResult(True, [])
        except Exception as e:
            self.logger.error(f"[BithumbSpot] {self.symbol} 오더북 업데이트 오류: {e}", exc_info=True)
...
class BithumbSpotOrderBookManager:
    ...
    def __init__(self, depth: int = 100, logger=None):
        """초기화"""
        self.depth = depth
        self.logger = logger
    ...
    async def fetch_snapshot(self, symbol: str) -> dict:
        ...
        while retry_count < max_retries:
            try:
                self.logger.debug(f"[BithumbSpot] {symbol} 스냅샷 요청 시도 ({retry_count+1}/{max_retries})")
                snapshot = await self._fetch_snapshot_impl(symbol)
                if snapshot and "data" in snapshot:
                    self.logger.debug(f"[BithumbSpot] {symbol} 스냅샷 요청 성공")
                    return snapshot["data"]
                else:
                    self.logger.warning(f"[BithumbSpot] {symbol} 스냅샷 응답 형식 오류")
            except Exception as e:
                self.logger.error(f"[BithumbSpot] {symbol} 스냅샷 요청 오류: {e}", exc_info=True)
            
            retry_count += 1
            await asyncio.sleep(1)  # 재시도 전 1초 대기
        
        self.logger.error(f"[BithumbSpot] {symbol} 스냅샷 요청 최대 재시도 횟수 초과")
        return None
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if data.get("status") == "0000":
                        ...
                        self.logger.error(f"[BithumbSpot] {symbol} API 오류: {data.get('message')}")
                else:
                    self.logger.error(f"[BithumbSpot] {symbol} HTTP 오류: {response.status}")
        ...
        try:
            ...
            for bid_item in bids_data:
                ...
                try:
                    ...
                    self.logger.warning(f"[BithumbSpot] 유효하지 않은 매수 데이터: {bid_item}")
                    continue
            ...
            for ask_item in asks_data:
                ...
                try:
                    ...
                    self.logger.warning(f"[BithumbSpot] 유효하지 않은 매도 데이터: {ask_item}")
                    continue
            ...
            # 디버깅을 위한 로깅 추가
            self.logger.debug(f"[BithumbSpot] {symbol} 스냅샷 파싱 완료: 매수 {len(bids)}개, 매도 {len(asks)}개")
            
            return parsed
        except Exception as e:
            self.logger.error(f"[BithumbSpot] 스냅샷 파싱 오류: {e}", exc_info=True)
            return {}
    ...
    async def initialize_orderbook(self, symbol: str):
        ...
        self.logger.info(f"[BithumbSpot] {symbol} 오더북 초기화 시작")
        ...
        if not snapshot:
            self.logger.error(f"[BithumbSpot] {symbol} 스냅샷 요청 실패")
            return
        ...
        self.logger.info(
            f"[BithumbSpot] {symbol} 오더북 초기화 완료 | "
        ...
        if self.output_queue:
            ...
            try:
                ...
                self.logger.debug(f"[BithumbSpot] {symbol} 스냅샷 큐 전송 완료")
    ...
    async def update(self, symbol: str, data: dict) -> ValidationResult:
        ...
        if symbol not in self.orderbooks:
            self.logger.warning(f"[BithumbSpot] {symbol} 오더북이 초기화되지 않음")
        ...
        if not st or not st["initialized"]:
            ...
            self.logger.debug(f"[BithumbSpot] {symbol} 초기화 전 이벤트 버퍼링 (버퍼 크기: {len(self.buffer_events[symbol])})")
        ...
        if timestamp <= last_id:
            self.logger.debug(f"[BithumbSpot] {symbol} 이미 처리된 이벤트 스킵 | timestamp={timestamp}, last_id={last_id}")
            ...
            if res.is_valid:
                ...
                if hasattr(ob, 'bids_dict') and hasattr(ob, 'asks_dict'):
                    ...
                    if bids_diff != 0 or asks_diff != 0:
                        self.logger.debug(
                            f"[BithumbSpot] {symbol} 오더북 크기 변화 | "
        ...
        if applied_count > 0 and self.output_queue:
            ...
            try:
                ...
                if current_time - last_log_time >= self.queue_log_interval:
                    ...
                    if hasattr(ob, 'bids_dict') and hasattr(ob, 'asks_dict'):
                        self.logger.debug(
                            f"[BithumbSpot] {symbol} 오더북 큐 전송 완료 | "
    ...
    async def subscribe(self, symbol: str):
        ...
        if self.ws:
            ...
            self.logger.info(f"[BithumbSpot] {symbol} 웹소켓 구독 완료")
        else:
            self.logger.error(f"[BithumbSpot] {symbol} 웹소켓 구독 실패: 웹소켓 연결 없음")
    ...
    def set_websocket(self, ws):
        """웹소켓 연결 설정"""
        self.ws = ws
        self.logger.info("[BithumbSpot] 웹소켓 연결 설정 완료")
