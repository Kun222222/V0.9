...
from crosskimp.ob_collector.utils.config.constants import (
    LOG_SYSTEM, CONFIG_DIR, SETTINGS_FILE, API_KEYS_FILE, BACKUP_DIR,
    SETTINGS_PATH, API_KEYS_PATH, LOAD_TIMEOUT, SAVE_TIMEOUT, RETRY_DELAY, MAX_RETRIES,
...
class ConfigManager:
    ...
    def _start_file_watching(self) -> None:
        ...
        self._observer.schedule(self._event_handler, path=CONFIG_DIR, recursive=False)
    ...
    async def load_api_keys(self) -> Dict:
        ...
        for exchange, env_vars in API_ENV_VARS.items():
            ...
            if exchange == "telegram":
            ...
            elif exchange == "security":
                # 보안 관련 설정
                access_token_secret = os.getenv(env_vars["access_token_secret"])
                if not access_token_secret:
                    logger.warning(f"{LOG_SYSTEM} 보안 설정(access_token_secret) 누락")
                else:
                    api_keys[exchange] = {
                ...
                if not all([first_superuser, first_superuser_password]):
                    logger.warning(f"{LOG_SYSTEM} 사용자 설정(first_superuser, first_superuser_password) 누락")
                else:
                    api_keys[exchange] = {
                ...
                if not all([api_key, api_secret]):
                    # 경고만 출력하고 계속 진행 (필수가 아닐 수 있음)
                    logger.warning(f"{LOG_SYSTEM} {exchange.title()} API 키 설정 누락")
                else:
                    api_keys[exchange] = {
                        "api_key": api_key,
