...
class OptimizedKimpProcessor:
    ...
    def __init__(self, apply_fee_discount=True):
        ...
        self.exchange_fees = {}
        for exchange in self.domestic_exchanges + self.foreign_exchanges:
            self.exchange_fees[exchange] = 0.001  # 0.1%
        
        # 기본 전송 수수료
        self.transfer_fee = 0.0002  # 0.02%
        ...
        # 중복 거래 방지를 위한 최근 거래 경로 캐싱
        self.recent_opportunities = {}  # 키: "전송코인_수익코인_전송거래소_수익거래소", 값: 타임스탬프
        self.opportunity_cooldown = 60  # 같은 경로 재사용까지 대기 시간(초)
    ...
    def _update_premium_for_pair(self, c_idx: int, d_idx: int, f_idx: int):
        ...
        with self._time_method('_update_premium_for_pair'):
            ...
            for_ask = for_data['sell']['avg_price']  # 해외 매도 호가
    ...
    def _find_best_opportunity(self) -> Optional[Dict[str, Any]]:
        ...
        with self._time_method('_find_best_opportunity'):
            ...
            # 수수료 계산 (각 거래소별 수수료)
            transfer_dom_fee = self.exchange_fees.get(transfer_dom, 0.001)
            transfer_for_fee = self.exchange_fees.get(transfer_for, 0.001)
            profit_dom_fee = self.exchange_fees.get(profit_dom, 0.001)
            profit_for_fee = self.exchange_fees.get(profit_for, 0.001)
            
            # 총 수수료 계산 (거래소 수수료 + 전송 수수료)
            # 분석 단계에서는 수수료를 좀 더 낮게 설정 (가능성 있는 기회를 더 많이 발견하기 위해)
            original_total_fee = transfer_dom_fee + transfer_for_fee + profit_dom_fee + profit_for_fee + self.transfer_fee
            
            # 수수료 할인 적용 (옵션에 따라)
            if self.apply_fee_discount:
                reduced_total_fee = original_total_fee * 0.01  # 실제 수수료의 1%만 적용 (99% 할인)
                total_fee = reduced_total_fee
                
                # 할인된 수수료 로깅 (조건문 내부로 이동)
                if self.perf_stats['process_count'] % 1000 == 0:
                    logging.debug(f"수수료 계산: 원래 수수료 {original_total_fee:.6f}, 할인 후 {reduced_total_fee:.6f}")
            else:
            ...
            # 김프 차이가 매우 작으면 제외
            if self.apply_fee_discount:
                # 테스트 모드: 완화된 기준 (-0.5%만 넘으면 OK)
                if premium_diff <= -0.005:
                    return None
            else:
            ...
            # 중복 거래 방지를 위한 확인 (백테스트에서는 시간 간격 줄임)
            current_time = time.time()
            if opportunity_path in self.recent_opportunities:
                last_time = self.recent_opportunities[opportunity_path]
                if current_time - last_time < 10:  # 10초 간격으로 완화
                    logging.debug(f"중복 거래 경로 무시: {opportunity_path}, 마지막 발견: {datetime.fromtimestamp(last_time)}")
                    return None
            
            # 최근 거래 경로 업데이트
            self.recent_opportunities[opportunity_path] = current_time
            ...
            logging.log(log_level, f"거래 기회({profit_status}): {transfer_coin}->{profit_coin} (전송: {transfer_premium:.4%}, 수익: {profit_premium:.4%}, 차이: {premium_diff:.4%}, 순이익: {net_profit:.4%}, 원본수수료: {original_total_fee:.4%}, 할인수수료: {reduced_total_fee:.4%})")
